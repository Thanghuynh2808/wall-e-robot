...
fp32 params_L[3] = {9.5f,  42.0f, 0.0f};   // Giảm Kp: 11.5→9.5, Ki: 49.8→42.0
fp32 params_R[3] = {60.48f, 85.00f, 0.0f};  // Giữ nguyên bánh phải
// --- Feedforward: PWM tối thiểu để thắng lực ma sát tĩnh ---
#define FF_OFFSET_LEFT   42000.0f
#define FF_OFFSET_RIGHT  55000.0f
#define FF_OFFSET_TURN   50000.0f   // Dùng khi xoay tại chỗ
#define FF_SLOPE_LEFT    80.1f
#define FF_SLOPE_RIGHT   600.1f
#define FF_SLOPE_LEFT_BWD    40.0f
        ...
        if (data == '\r' || data == '\n') {
            ...
            if (rx_index > 0) {
                ...
                // Lệnh 'u': Cập nhật tham số PID từ ROS (u Kp:Kd:Ki:Ko)
                    ...
                    if (parsed >= 3) {
                        params_L[0] = (float)p; params_R[0] = (float)p;
                        params_L[1] = (float)i; params_R[1] = (float)i;
                        params_L[2] = (float)d; params_R[2] = (float)d;
                        PID_init(&pid_speed_left, PID_POSITION, params_L, 65535, 8500);
                        PID_init(&pid_speed_right, PID_POSITION, params_R, 65535, 10000);
                    }
                ...
                openloop_mode = 1;
                int32_t pwm = 0;
                sscanf(rx_buffer, "p %ld", &pwm);
                target_speed_left  = 0;
                target_speed_right = 0;
                // Cấp thẳng PWM, bypass PID
...
void StartChassisTask(void *argument) {
    for (;;) {
        ...
        if (debug_mode) {
            char raw_buf[32];
            int raw_len = sprintf(raw_buf, "%d,%d\n", delta_left, delta_right);
            CDC_Transmit_FS((uint8_t*)raw_buf, raw_len);
        }
        ...
        if (target_speed_left == 0 && target_speed_right == 0) {
            ...
            PID_clear(&pid_speed_right);
						...
						float ff_l = 0, ff_r = 0;
						float cur_offset_l = FF_OFFSET_LEFT;
						float cur_offset_r = FF_OFFSET_RIGHT;

						// Tự động nhận diện chế độ Xoay (hai bánh ngược chiều)
						if (target_speed_left * target_speed_right < 0) {
								cur_offset_l = FF_OFFSET_TURN;
								cur_offset_r = FF_OFFSET_TURN;
						}

						if (target_speed_left > 0)
								ff_l =  cur_offset_l + target_speed_left * FF_SLOPE_LEFT;
						else if (target_speed_left < 0)
								ff_l = -(cur_offset_l + (-target_speed_left) * FF_SLOPE_LEFT_BWD);

						if (target_speed_right > 0)
								ff_r =  cur_offset_r + target_speed_right * FF_SLOPE_RIGHT;
						else if (target_speed_right < 0)
								ff_r = -(cur_offset_r + (-target_speed_right) * FF_SLOPE_RIGHT);

						float pid_l = PID_calc(&pid_speed_left,  (float)delta_left,  target_speed_left);
						float pid_r = PID_calc(&pid_speed_right, (float)delta_right, target_speed_right);
						...
						if (target_speed_left == 0) {
								...
								out_l = (int32_t)(ff_l + pid_l);
								if (target_speed_left > 0 && out_l < 0) out_l = 0;
								if (target_speed_left < 0 && out_l > 0) out_l = 0;
						}
						...
						if (target_speed_right == 0) {
								...
								out_r = (int32_t)(ff_r + pid_r);
								if (target_speed_right > 0 && out_r < 0) out_r = 0;
								if (target_speed_right < 0 && out_r > 0) out_r = 0;
						}
