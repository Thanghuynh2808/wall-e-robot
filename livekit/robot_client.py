...
BLOCKSIZE      = 8192
# Pre-buffer: accumulate before starting ALSA playback.
# Must be larger than worst-case LiveKit cloud jitter to prevent underruns.
# After multiple POIs, the relay path (Vietnam → Hyderabad) can spike to
# 300-500ms jitter, so 150ms was too small → underruns grew worse over time.
# 1000ms matches the speech-ctrl lead-in silence perfectly: the pre-buffer
# fills exactly during lead-in, so speech starts without any extra delay.
...
async def _livekit_connect(robot_id: str, player: AudioPlayer):
    ...
    while not _stop_event.is_set():
        ...
        try:
            ...
            while not _stop_event.is_set() and not _disc_event.is_set():
                await asyncio.sleep(1.0)
